...
try:
    ...
    import_msg_main = traceback.format_exc()
    try:
        from function import handler
        main = handler
    except:
        import_msg_function = traceback.format_exc()
        import_msg = import_msg_main + '\n' + import_msg_function
...
def append_metadata(user_ret, start, end, was_cold, success=True):
    ...
    ret = {"start": datetime.strftime(start, FORMAT), "end": datetime.strftime(
        end, FORMAT), "was_cold": was_cold, "duration_sec": duration}
...
def invoke():
    ...
    try:
        ...
        # return formating for sebs functions
        if 'handler' in globals().keys():
            ...
            return append_metadata(user_result, start, end, was_cold)
        else:
            # wrap user output with our own recorded information
