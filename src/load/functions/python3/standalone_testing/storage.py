...
class storage:
    ...
    def upload(self, bucket, key_name, filepath):
        client = self.client

        if not client.bucket_exists(bucket):
            client.make_bucket(bucket)

        client.fput_object(bucket, key_name, filepath)
    ...
    def upload_stream(self, bucket, file, bytes_data):
        ...
        self.client.put_object(bucket, key_name, bytes_data,
                               bytes_data.getbuffer().nbytes)
        return key_name
