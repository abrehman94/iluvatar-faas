...
def handler(event):
    ...
    client.download_directory(bucket, os.path.join(
        input_prefix, key), download_path)
    s3_download_stop = datetime.datetime.now()
    size = parse_directory(download_path)

    compress_begin = datetime.datetime.now()
    shutil.make_archive(os.path.join(download_path, key),
                        'zip', root_dir=download_path)
    ...
    key_name = client.upload(bucket, os.path.join(
        output_prefix, archive_name), os.path.join(download_path, archive_name))
