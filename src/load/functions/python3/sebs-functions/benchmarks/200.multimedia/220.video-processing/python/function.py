...
def watermark(video, duration, event):
    ...
    call_ffmpeg([
        "-i", video,
        "-i", os.path.join(watermark_file,
                           os.path.join('resources', 'watermark.png')),
        "-t", "{0}".format(duration),
...
operations = {'transcode': transcode_mp3,
              'extract-gif': to_gif, 'watermark': watermark}
...
def handler(event):
    ...
    upload_key = client.upload(bucket, os.path.join(
        output_prefix, filename), upload_path)
