...
def handler(event):
    ...
    key_name = client.upload_stream(
        bucket, os.path.join(output_prefix, key), resized)
    upload_end = datetime.datetime.now()

    download_time = (download_end - download_begin) / \
        datetime.timedelta(microseconds=1)
    upload_time = (upload_end - upload_begin) / \
        datetime.timedelta(microseconds=1)
