...
if os.path.exists('function/torch.zip'):
    ...
    sys.path.append(os.path.join(os.path.dirname(__file__),
                    '/tmp/.python_packages/lib/site-packages'))
...
def handler(event):
    ...
    if not model:
        ...
        client.download(bucket, os.path.join(
            model_prefix, model_key), model_path)
    ...
    download_time = (image_download_end - image_download_begin) / \
