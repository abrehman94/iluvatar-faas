...
pub struct PreAllocatedGroups {
    ...
    groups: Arc<DashMap<SchedGroupID, Vec<u32>>>,
...
impl PreAllocatedGroups {
    pub fn new( sm: Arc<SharedMapsSafe>, groups: Vec<Vec<u32>>, timeslices: Vec<u64>  ) -> PreAllocatedGroups {
        
        // build hashmap of groups 
        let gh: DashMap<SchedGroupID,Vec<u32>> = groups.into_iter().enumerate().map(|x|{
            let (a,b) = x;
            (a as SchedGroupID, b)
        }).collect();

        // populate the sharedmap with it 
        gh.iter().zip(timeslices.iter()).for_each( |(ent,ts)| {
    ...
    pub fn groups(&self) -> Arc<DashMap<SchedGroupID, Vec<u32>>> {
        self.groups.clone()
    }
